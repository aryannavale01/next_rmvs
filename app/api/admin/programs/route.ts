...
import { createProgramSchema } from '@/lib/validations/admin-program';
...
const QuerySchema = z.object({
  ...
  visibility:     z.enum(['homepage', 'programs', 'both']).optional(),
  page:           z.coerce.number().int().min(1).default(1),
  pageSize:       z.coerce.number().int().min(1).max(100).default(20),
  sortBy:         z.enum(['title', 'category', 'isStrategic', 'createdAt', 'updatedAt']).default('createdAt'),
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (q.isStrategic !== undefined) where.isStrategic = q.isStrategic;
    if (q.visibility) where.visibility = q.visibility;

    const skip = (q.page - 1) * q.pageSize;
    const [data, total] = await withRetry(() =>
      Promise.all([
        prisma.program.findMany({ where, orderBy: { [q.sortBy]: q.sortOrder }, skip, take: q.pageSize }),
    ...
    console.error('[GET /api/admin/programs]', error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const program = await withRetry(() => prisma.program.create({ data: parsed.data }));
    ...
    console.error('[POST /api/admin/programs]', error);
    if (isTransientPrismaError(error)) {
      return NextResponse.json({ error: 'Database temporarily unavailable, please retry.' }, { status: 503 });
    }
