...
import { 
  ...
  Lightbulb, Globe, CheckCircle, Sparkles, AlertTriangle, X
} from 'lucide-react';
import { courses, strategicPrograms } from '@/lib/public-data';
import { Course } from '@/lib/public-data';
...
export default function ProgramsPage() {
  ...
  const [enrollForm, setEnrollForm] = useState({ name: '', email: '', motivation: '', agree: false });
  ...
  }, [searchQuery, activeCourseCategory]);

  // Filter field programs
  ...
  const handleEnrollSubmit = (e: FormEvent) => {
    e.preventDefault();
    if (enrollForm.name && enrollForm.email && enrollForm.agree) {
      setEnrollSuccess(true);
      setTimeout(() => {
        setEnrollSuccess(false);
        setEnrollingCourse(null);
        setEnrollForm({ name: '', email: '', motivation: '', agree: false });
      }, 3500);
    }
  };
  ...
  return (
    <div className="space-y-0" id="programs-page-root">
      ...
      {/* VIEW 1: TRAINING & CERTIFICATION CATALOG (IMAGE 2) */}
      {activeTab === 'training' && (
              ...
              {filteredCourses.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8" id="course-cards-grid">
                  {filteredCourses.map((c) => (
                    <div key={c.id} className="group bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col justify-between" id={`course-card-${c.id}`}>
                      <div>
                      ...
                      {/* Instructor, specs & Action button */}
                      <div className="p-6 pt-0 space-y-4">
                        ...
                        <button 
                          ...
                          className="w-full inline-flex items-center justify-center px-4 py-3 bg-brand-primary hover:bg-brand-primary-hover text-white font-semibold text-sm rounded-xl transition-all shadow-sm hover:shadow-md cursor-pointer"
                          id={`btn-course-enroll-${c.id}`}
                        >
                          Enroll Now
                        </button>
                      </div>
      ...
      {/* VIEW 2: STRATEGIC PROGRAMS & OPERATIONS (IMAGE 7) */}
      {activeTab === 'field' && (
              ...
              <div className="bg-[#FAFBF9] rounded-[2rem] border border-gray-100 overflow-hidden shadow-sm grid grid-cols-1 lg:grid-cols-12" id="featured-field-box">
                ...
                <div className="lg:col-span-6 p-8 sm:p-12 flex flex-col justify-between space-y-6">
                  <div className="space-y-4">
                    ...
                    <h2 className="font-display font-bold text-2xl sm:text-3xl text-gray-900 tracking-tight">The Great Green Wall Restoration</h2>
                    <p className="text-sm text-gray-500 leading-relaxed">
                      Our flagship environmental program aims to restore 100,000 hectares of vital ecosystems across the Sahel region. By integrating traditional silviculture with modern tech mapping, we are creating native tree barriers to halt desertification and fuel rural farming.
                    </p>
                  </div>
                  ...
                  <div className="space-y-4 pt-2 border-t border-gray-100">
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-xs font-semibold">
                        <span className="text-gray-500">Goal: $2.5M USD</span>
                        <span className="text-brand-primary">82% Funded</span>
                      </div>
                      <div className="w-full h-2.5 bg-gray-150 rounded-full overflow-hidden">
                        <div className="h-full bg-brand-primary rounded-full" style={{ width: '82%' }} />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 pt-2">
                      <button 
                        ...
                        className="px-6 py-3 bg-brand-primary hover:bg-brand-primary-hover text-white font-semibold text-xs rounded-xl shadow-sm hover:shadow-md transition-all cursor-pointer flex items-center gap-1.5"
                      >
                        Donate to this Program
                      </button>
                      <button 
                        onClick={() => toast({ title: 'Impact Story', description: 'Restoration progress report of Sahel region: Reforestation of 12,000 hectares completed, utilizing native acacia. Next phase launched in Senegal. Project audit: fully compliant.', variant: 'info' })}
                        className="px-5 py-3 bg-white hover:bg-gray-50 border border-gray-200 text-gray-700 font-semibold text-xs rounded-xl shadow-sm transition-all cursor-pointer"
                      >
                        Read Impact Story
                      </button>
                    </div>
                  ...
                  return (
                    <div key={p.id} className="group bg-[#FAFBF9] rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between" id={`field-card-${p.id}`}>
                      ...
                      <div className="p-6 pt-0 space-y-4">
                        ...
                        <button 
                          onClick={() => router.push('/donate')}
                          className="w-full py-2.5 bg-white hover:bg-gray-50 border border-gray-200 text-gray-800 font-semibold text-xs rounded-xl shadow-sm transition-colors cursor-pointer text-center block"
                        >
                          Donate Today
                        </button>
                      </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="bg-brand-navy rounded-[2.5rem] p-8 sm:p-16 text-white text-center space-y-6 relative overflow-hidden" id="amplifying-box">
                ...
                <div className="flex flex-wrap justify-center gap-4 pt-4">
                  <button 
                    onClick={() => toast({ title: 'Partner Request', description: 'Partner application requested! Our corporate advocacy office will reach out within 24 hours.', variant: 'success' })}
                  ...
                  <button 
                    onClick={() => toast({ title: 'Contact Opened', description: 'Connect directly with partnerships@compassionglobal.org', variant: 'info' })}
                    className="px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold text-xs rounded-xl transition-all cursor-pointer uppercase tracking-wider"
                  >
      ...
      {/* MODAL: ENROLLMENT FORM POPUP */}
      {enrollingCourse && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200" id="modal-enroll">
          <div className="bg-white rounded-3xl overflow-hidden max-w-md w-full shadow-2xl border border-gray-100 animate-in zoom-in-95 duration-200 relative">
            <button 
              onClick={() => { if (!enrollSuccess) setEnrollingCourse(null); }}
            ...
            <div className="p-8 space-y-6">
              <div className="space-y-2">
                <span className="text-[10px] font-mono font-bold tracking-widest text-emerald-600 bg-emerald-50 px-2.5 py-0.5 rounded-full uppercase">
                  {enrollingCourse.category} Academy
                </span>
                <h3 className="font-display font-bold text-xl text-gray-900">Enrollment Portal</h3>
                <p className="text-xs text-gray-400">
                  Registering for: <span className="font-semibold text-gray-700">{enrollingCourse.title}</span>
                </p>
              </div>

              {enrollSuccess ? (
                <div className="p-6 bg-emerald-50 border border-emerald-100 rounded-2xl text-center space-y-3 animate-in zoom-in-95" id="enroll-form-success">
                  <CheckCircle className="h-10 w-10 text-brand-primary mx-auto" />
                  <h4 className="font-display font-bold text-base text-gray-900">Application Approved!</h4>
                  <p className="text-xs text-gray-500 leading-relaxed">
                    Thank you, <span className="font-semibold text-gray-800">{enrollForm.name}</span>. A confirmation welcome packet with course syllabus has been sent to <span className="font-semibold text-gray-800">{enrollForm.email}</span>.
                  </p>
                </div>
              ) : (
                <form onSubmit={handleEnrollSubmit} className="space-y-4" id="enroll-form">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary transition-all"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-gray-700 uppercase tracking-wider block">Statement of Purpose (Optional)</label>
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary transition-all resize-none"
                    />
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 pt-2">
                    <input
                      ...
                      id="enroll-agree"
                      required
                      checked={enrollForm.agree}
                      onChange={(e) => setEnrollForm({ ...enrollForm, agree: e.target.checked })}
                      className="mt-1 h-4 w-4 rounded border-gray-300 text-brand-primary focus:ring-brand-primary/20 cursor-pointer"
                    />
                    <label htmlFor="enroll-agree" className="text-xs text-gray-500 leading-normal cursor-pointer select-none">
                      I agree to the <span className="font-semibold text-gray-700 hover:underline">Academic Terms of Use</span> and consent to receive course administration updates.
                    </label>
                  </div>

                  <button
                    type="submit"
                    className="w-full py-3 bg-brand-primary hover:bg-brand-primary-hover text-white font-semibold text-sm rounded-xl transition-colors cursor-pointer mt-4"
                  >
                    Submit Application
                  </button>
                </form>
