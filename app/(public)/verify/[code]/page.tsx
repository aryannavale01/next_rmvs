...
import { Shield, ShieldCheck, ShieldAlert, ShieldX, ScanSearch, BadgeCheck } from "lucide-react";
...
export async function generateMetadata(): Promise<Metadata> {
  return generatePageMetadata({
    ...
    description:
      "Verify the authenticity of a certificate issued by Rupashree Mahila Vikas Sanstha (RMVS) under the MH-SKILL program. Enter or scan the verification code.",
    path: "/verify",
  });
}
...
export default async function VerifyPage({ params }: VerifyParams) {
  ...
  const [orgConfig] = await Promise.all([getOrgConfig()]);
  ...
  if (rawCode.length < 8 || rawCode.length > 64) {
    notFound();
  }
  ...
  try {
    cert = await withRetry(() =>
      prisma.certificate.findUnique({
        where: { verificationCode: rawCode },
        select: {
          id: true,
  ...
  const qrDataUrl = await QRCode.toDataURL(
    `${getVerificationBaseUrl()}/verify/${rawCode}`,
    { width: 228, margin: 1, color: { dark: "#0f172a", light: "#ffffff" } },
  ).catch(() => null);

  const isRevoked = cert.status === "revoked" || cert.revokedAt !== null;
  const isValid = !isRevoked && cert.publishedStatus === "published";

  const orgShort = orgConfig.siteName?.split(" ")[0] || "RMVS";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-10">
      <div className="max-w-lg mx-auto px-4">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-500">{orgConfig.siteName} — MH-SKILL Program</p>
        </div>
            ...
            <div>
              ...
              >
                {isValid
                  ...
                  : "This certificate has been generated but not yet published."}
              </p>
            </div>
          </div>
          ...
          {qrDataUrl && (
            <div className="flex items-center justify-center pt-5">
              <div className="p-2 border border-slate-200 rounded-xl bg-white shadow-sm">
                ...
                <img
                  src={qrDataUrl}
                  alt={`QR code linking to verification of ${cert.certificateNumber}`}
                  className="w-28 h-28"
                />
              </div>
            </div>
        ...
        <div className="text-center mt-6 space-y-3">
          <Link
            href="/verify"
            className="inline-flex items-center justify-center gap-2 w-full py-2.5 px-4 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-hover transition-colors"
          >
            <ScanSearch size={16} />
            Verify Another Certificate
          </Link>
          <p className="text-xs text-slate-400">
            Verified at {new Date().toLocaleString("en-IN")} — {orgShort} ({orgConfig.siteName})
          </p>
        </div>
      </div>
    </div>
