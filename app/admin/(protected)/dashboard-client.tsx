...
interface RecentMember { id: string; fullName: string; district: string | null; createdAt: string; }
interface RecentCert { id: string; certificateNo: string; status: string; memberName: string; createdAt: string; }
interface RecentNotif { id: string; title: string; description: string; }
interface ActivityLog { id: string; title: string; description: string; timestamp: string; icon: string; }

interface ChartDataPoint { month: string; value: number; }
interface Props {
  ...
  chartData?: { bar: ChartDataPoint[]; line: ChartDataPoint[]; area: ChartDataPoint[] };
}

export default function DashboardClient({ counts, recentMembers, recentCertificates, recentNotifications, activityLogs, chartData }: Props) {
  ...
  const barChartData = chartData?.bar ?? [
    ...
    { month: 'Apr', value: 0 }, { month: 'May', value: 0 }, { month: 'Jun', value: 0 }, { month: 'Jul', value: 0 },
  ];
  const lineChartData = chartData?.line ?? [
    { month: 'Jan', value: 0 }, { month: 'Feb', value: 0 }, { month: 'Mar', value: 0 },
    { month: 'Apr', value: 0 }, { month: 'May', value: 0 }, { month: 'Jun', value: 0 }, { month: 'Jul', value: 0 },
  ];
  const areaChartData = chartData?.area ?? [
    { month: 'Jan', value: 0 }, { month: 'Feb', value: 0 }, { month: 'Mar', value: 0 },
    { month: 'Apr', value: 0 }, { month: 'May', value: 0 }, { month: 'Jun', value: 0 }, { month: 'Jul', value: 0 },
  ];
  ...
  return (
    <div className="space-y-6 font-sans">
      <MetricCards
        ...
        columns={4}
        cards={[
          { id: 'members', label: 'Total Members', value: counts.totalMembers, icon: Users },
          { id: 'courses', label: 'Active Trainings', value: counts.activeCourses, icon: BookOpen },
          { id: 'certs', label: 'Certificates', value: counts.totalCerts, icon: Award },
          { id: 'pending', label: 'Pending Apps', value: counts.pendingEnrollments, icon: Calendar },
          { id: 'teachers', label: 'NGO Teachers', value: counts.totalTeachers, icon: Layers },
          { id: 'enrollments', label: 'Enrollments', value: counts.totalEnrollments, icon: FileSpreadsheet },
          { id: 'coupons', label: 'Coupons', value: counts.totalCoupons, icon: Percent },
          { id: 'alerts', label: 'Alerts Sent', value: counts.totalNotifs, icon: Bell },
        ]}
      />
          ...
          <div className="h-44 w-full">
            <svg className="w-full h-full" viewBox="0 0 300 160">
              ...
              {barChartData.map((d, i) => {
                ...
                const barMax = Math.max(40, ...barChartData.map(b => b.value));
                const barHeight = (d.value / barMax) * 120;
        ...
        <div className="bg-card border border-border p-5 rounded-xl">
          ...
          <div className="h-44 w-full">
            <svg className="w-full h-full" viewBox="0 0 300 160">
              ...
              <line x1="0" y1="140" x2="300" y2="140" stroke="#E2E8F0" strokeWidth="1" />
              {(() => {
                const lineMax = Math.max(30, ...lineChartData.map(d => d.value));
                const points = lineChartData.map((d, i) => ({
                  x: (300 / (lineChartData.length - 1)) * i,
                  y: 140 - (d.value / lineMax) * 120,
                }));
        ...
        <div className="bg-card border border-border p-5 rounded-xl">
          ...
          <div className="h-44 w-full">
            <svg className="w-full h-full" viewBox="0 0 300 160">
              ...
              <line x1="0" y1="140" x2="300" y2="140" stroke="#E2E8F0" strokeWidth="1" />
              {(() => {
                const areaMax = Math.max(150, ...areaChartData.map(d => d.value));
                const points = areaChartData.map((d, i) => ({
                  x: (300 / (areaChartData.length - 1)) * i,
                  y: 140 - (d.value / areaMax) * 120,
                }));
                ...
                {recentCertificates.map(c => (
                  <div key={c.id} className="flex justify-between items-center text-xs pb-2.5 border-b border-border last:border-0 last:pb-0">
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground font-mono">{c.certificateNo}</p>
                    </div>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${c.status === 'accepted' ? 'text-green-600 bg-green-50' : 'text-yellow-600 bg-yellow-50'}`}>{c.status}</span>
                  </div>
                ))}
              </div>
            </div>
